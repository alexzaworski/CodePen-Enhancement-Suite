...
var ces__inlineProfiles = (function(){
  ...
  function preview(profileLink){
    ...
    _this.addProfileToPreview = function(){
      ...
      if (profile.isPro) {
        name.append($("<span class='badge badge-pro'>Pro</span>"));
      }
      ...
      template.find(".ces__profile__follow-buttons").toggleClass("ces__isFollowing", profile.isFollowing);
      ...
      function handleFollowEvents(initialState) {
        ...
        if (_this.profile.username.substring(1) === initData.__user.username) {
          template.find(".ces__profile__follow-buttons").remove();
          return;
        }
        var state = initialState;
        template.find(".ces__profile__follow-buttons").click(function(){
          state = !state;
