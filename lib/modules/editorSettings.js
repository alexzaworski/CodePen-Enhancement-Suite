  ...
  u_requestExtensionUrl("lib/html/editor-settings.html", function(response){
    $editorSettings.load(response, function(){
      ...
      if (initData.__pen.slug_hash === "") {
        $editorSettings.prepend("<p>Editor settings are only available once you've saved your Pen at least once.</p>");
        $editorSettings.find("input, select, label").addClass("disabled");
        return;
      }
